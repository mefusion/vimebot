...
// И да, я программист
// Доведено до ума by Fusion Prime
...
Bot.on("guildMemberAdd", async (member) => {
  var msg = `Здравствуй, <@${member.id}>! Для вывода списка команд используй: ${
    config[member.guild.id].prefix
  }help`;
  if (config[member.guild.id].botchannel != "*")
    return member.guild.channels
      .find("id", String(config[member.guild.id].botchannel))
      .send(msg);
...
Bot.on("message", async (message) => {
  ...
  if (
    ...
    return false;
  if (
    String(message.channel.id) != config[server.id].botchannel &&
    config[server.id].botchannel != "*"
  ) {
    message.reply(
      "Вы пишите не в том текстовом канале. Лучше перейдите в <#" +
        config[server.id].botchannel +
        ">"
    );
    return false;
...
Я робот, у меня нет эмоций.\`\``);
    ...
    case "online":
      ...
      req.send();
      if (req.status == 200) var data = JSON.parse(req.responseText);
      for (var i = 0; i < data.length; i++) {
        if (i == data.length - 1) moders += "`" + data[i].username + "`.";
        else moders += "`" + data[i].username + "`, ";
      }
      var cd =
        "Всего модераторов онлайн " /* дальше будут показыватся модеры и их кол-во */ +
        data.length +
        ";\n" +
        moders;
    ...
    case "streams":
      ...
      if (req.status == 200) var data = JSON.parse(req.responseText);
      for (var i = 0; i < data.length; i++) {
        if (i == data.length - 1) streams += "**" + data[i].title + "**.";
        else streams += "**" + data[i].title + "**, ";
      }
      var cd =
        "Всего стримов онлайн " /* дальше будут показыватся стримы и их кол-во */ +
        data.length +
        ";\n" +
        streams;
      message.channel.send(cd);
  ...
  if (msg.startsWith("stats")) {
    ...
    req.send();
    if (req.status == 200) var data = JSON.parse(req.responseText)[0];
    if (!data) {
    ...
    req.open(
      "GET",
      "http://api.vime.world/user/" + data.id + "/session",
      false
    );
    req.send();
    if (req.status == 200) var data1 = JSON.parse(req.responseText);
    message.channel.send(
      "Ник: [`" +
        data.rank +
        "`] `" +
        ...
        "\n" +
        (data.guild
          ? "Состоит в гильдии " + data.guild.name
          : "Не состоит в гильдии") +
        "\nСыграно около " +
        Math.round(data.playedSeconds / 60 / 60 / 24) +
        " дней\n\n" +
        (data1.online.value
          ? "Игрок онлайн. " + data1.online.message
          : data1.online.message)
    );
    ...
    req.open(
      "GET",
      "http://api.vime.world/guild/get?name=" + encodeURIComponent(args[1]),
      false
    );
    req.send();
    if (req.status == 200) var data = JSON.parse(req.responseText);
    ...
    message.channel.send(
      "Название: `" +
        ...
        perks +
        "\n\nТег: " +
        (data.tag ? data.tag : "Отсутствует") +
        "\nЦвет тега: " +
        (data.tag ? data.color : "Тег отсутствует") +
        "\n\nИгроков всего: " +
        data.members.length +
        "."
    );
    ...
    req.open(
      "GET",
      "http://api.vime.world/leaderboard/get/" + args[1] + "?size=10",
      false
    );
    req.send();
    if (req.status == 200) var data = JSON.parse(req.responseText);
    ...
    switch (args[1]) {
      case "set":
        ...
        fs.writeFileSync("configs.json", JSON.stringify(config));
        message.channel.send(
          "Префикс бота для сервера " +
            server.name +
            " [" +
            server.id +
            '] был успешно изменён на "' +
            args[2] +
            '"'
        );
        break;
